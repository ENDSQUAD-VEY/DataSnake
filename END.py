...
def connectDataBase(conn):
    ...
    log.logRastreio('connectDataBase: String Recebida', strconn)
    log.logRastreio('connectDataBase: Conectando ao banco de dados')
    try:
        connection = pyodbc.connect(strconn, autocommit = True)
...
def InsertIntoErrorTable(errorDescription, Error_DateTime, Email,conn):
    ...
    cursor = connectDataBase(strconn)
...
def csvToDataBase(fileName, tableName, Error_DateTime, Email2,conn):
    ...
    try:
        ...
        InsertIntoErrorTable(errodescript, Error_DateTime, Email2,conn)
        return 1
